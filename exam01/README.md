...
1. 모델 선택
    ...
    - Gemini 3 Pro (Flash): 
        - 추론 깊이: 중상 (Fast Agent)
        - 응답 속도: 매우 빠름
        - 추천 용도: 바이브 코딩, UI/UX 수정, 빠른 반복 테스트
1. 로컬 실행 환경 준비
...
- 설정한 이후 `data`와 `logs` 디렉토리를 만들어줘야 이후 과정에서 에이전트가 헤매지 않는다.
...
- 로그인 정보:
    - JDBC URL: `jdbc:h2:file:./data/testdb`
        - 만약 데이터베이스가 다른 컴퓨터에 있다면 아래처럼 입력
        - `jdbc:h2:tcp://[IP_ADDRESS]/testdb`
        - `jdbc:h2:http://[IP_ADDRESS]/testdb`
    - 사용자 이름: `sa`
    - 비밀번호: (빈칸)
    - Connect 버튼 클릭
    - 로그인이 안 된다면? 
...
- 여기에서 만들고 있는 프로젝트는 한글을 기입하는 형태를 사용하고 있지만 실무에서는 다국어 메시지를 적용해야 한다.
    - 다국어지원: internationalization(i18n)
    - 다국어화: localization(l10n)
    + Kubernetes(쿠버네티스, K8S): 컨테이너화된 애플리케이션을 배포, 확장, 관리하기 위한 오픈소스 시스템
- 강의자료에서의 디렉터리 구조 필시 암기하기.
    - 아키텍처 내의 디렉터리 명들은 정해져 있음.
- DTO cs Entity vs VO
